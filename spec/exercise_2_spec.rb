...
2) I then created a double for the Entry class itself. The double possesses a new property, which can also be considered a
   new method, and this returns the entry double we created on the previous line.
3) We set our Diary class to take in the Entry class or an Entry double, so we pass in our entry_class double to the Diary
   instance we will use for our test.
4) In our it block, we call the add method on the Diary instance. Which will call the new method on the entry_class double,
   which will return our entry double and push it into the @entries array.
5) We then call the index instance method, which prints all the entry titles, and since we only added 1 title, it should
